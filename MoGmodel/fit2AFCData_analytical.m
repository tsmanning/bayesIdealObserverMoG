function [pNuHat,pGamHat,pWHat,pSigNse1Hat,pSigNse2Hat,priHat,nll] = fit2AFCData_analytical(stim,r,nB,xgrid,prs0)

% Fit Bayesian ideal observer model with two Gaussian noise levels from observer
% 2AFC data, consisting of triplets of stimuli [x1,x2] and observer choices
% 'x2>x1: yes/no'. Analytically calculates p("yes"|x1,x2) with the
% approximate solution in equation 58 from the main paper text.
%
% Inputs:
% -------
...
if nargin < 5
    pNu0      = -1*ones(nB,1);                 % initial values of component centers
    pGam0     = ones(nB,1);                  % initial values of component widths
    pW0       = normpdf(1:nB,(nB+1)/2,nB/4)';
    pW0       = pW0./sum(pW0);               % initial value of MoG weights
    pSigNse0  = [0.2; 0.8];                  % initial estimate of noise stdev
...
LB  = [-100*ones(nB,1);   zeros(nB,1); zeros(nB,1);   zeros(2,1)]; % lower bound
UB  = [   0*ones(nB,1); 10*ones(nB,1);  ones(nB,1); 10*ones(2,1)]; % upper bound
...
% ----------------------------
% Extract fitted parameters & generate estimated prior
% ----------------------------
...
pSigNse1Hat = prshat(3*nB + 1);
pSigNse2Hat = prshat(3*nB + 2);
...
p = calcMoGPFxn_Analytic(pNu,pGam,pW,mu1,sig1,mu2,sig2);
